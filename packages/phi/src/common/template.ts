import { AbiFunction, Artifact, Contract, SignatureTemplate, Transaction, Utxo } from "cashscript";
...
export const buildAuthenticationTemplate = async ({
  contract,
  artifact,
  transaction,
  network,
  ...
  if (manglePrivateKeys === undefined && network !== Network.REGTEST) {
  ...
  const functionIndex = artifact.abi.findIndex(
    (val:any) => val.name === funcName
  )!;
  const func: AbiFunction = artifact.abi[functionIndex]!;
      ...
      evaluate_function: {
        data: {
          bytecode: merge([
            ...
            ...constructorInputs.map((input:any, index:any) => ({
              [snake_case(input.name)]:
                "0x" 
                +
                binToHex(
                  contractParameters[index]! as Uint8Array
                )
                ,
            })),
          ]),
        ...
        transaction: [libauthTransaction].map((val: any) => {
          ...
          result!.inputs = val!.inputs!.map((input:any, index:any) => {
        ...
        sourceOutputs: ((transaction as any).inputs as Utxo[]).map(
          (val, index) => {
            val;
            const result = {} as
              | AuthenticationTemplateScenarioSourceOutput
        ...
        script: [
            ...
            ? zip(functionInputs, args).map(([input, arg]) =>
                input.type === PrimitiveType.SIG
                  ...
                  : `<${snake_case(input.name)}> // ${input.type} = <${
                      "0x" +
                      binToHex(arg as Uint8Array)
                    }>`
              )
            : ["// none"]),
          "",

          ...(artifact.abi.length > 1
            ? [
          ...
          ...(constructorInputs.length
            ? constructorInputs.map((input, index) => {
              //@ts-ignore
                const encoded = contractParameters[index]! as Uint8Array;
