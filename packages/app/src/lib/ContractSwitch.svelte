<script lang="ts">
	...
	import DripForm from '$lib/forms/DripForm.svelte';
...
<div>
	{#if instanceType == 'Annuity'}
		<AnnuityForm bind:contract bind:network={nodeValue} bind:version />
	{:else if instanceType == 'Divide'}
		<DivideForm bind:contract bind:network={nodeValue} bind:version />
	{:else if instanceType == 'DripMine'}
		<DripForm bind:contract bind:network={nodeValue} bind:version />
	{:else if instanceType == 'Faucet'}
		<FaucetForm bind:contract bind:network={nodeValue} bind:version />
	{:else if instanceType == 'Perpetuity'}
		<PerpetuityForm bind:contract bind:network={nodeValue} bind:version />
	{:else if instanceType == 'Mine'}
		<MineForm bind:contract bind:network={nodeValue} bind:version />
	{:else if instanceType == 'Record'}
		<RecordForm bind:contract bind:network={nodeValue} bind:version />
	{:else}
