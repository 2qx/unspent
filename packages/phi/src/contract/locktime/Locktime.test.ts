...
describe.skip(`Locktime Class Tests`, () => {
  test("Should a serialize a Locktime", async () => {
    ...
    let c2 = Locktime.fromString(c.toString());
  ...
  test("Should a deserialize and reserialize a chipnet Locktime", async () => {
    ...
    let c2 = Locktime.fromString(c.toString(), "chipnet");
  ...
  test("Should a deserialize and reserialize a regtest Locktime to and from an opreturn", async () => {
    ...
    let c2 = Locktime.fromOpReturn(opReturn, "regtest");
  ...
  test("Should pay a Locktime", async () => {
    ...
    let options = { version: 1, network: "regtest" };
    let c1 = new Locktime(1n, bob.getDepositAddress(), Locktime.minAllowance, options);
