...
export class Divide extends BaseUtxPhiContract implements UtxPhiIface {
    ...
    constructor(
        ...
        if (!(divisor >= 2 && divisor <= 4)) throw Error(`Divide contract range must be 2-4, ${(divisor)} out of range`)
    ...
    static fromString(str: string, network = "mainnet"): Divide {
        ...
        if (!(Divide.c == p.code)) throw ("non-faucet serialized string passed to faucet constructor")
    ...
    static fromOpReturn(opReturn: Uint8Array | string, network = "mainnet"): Divide {
    ...
    async execute(exAddress?: string, fee?: number, utxos?: Utxo[]): Promise<string> {
        ...
        if (exAddress) {
            ...
            to.pop()
            to.push({
                to: exAddress, amount: this.executorAllowance - (fee + 20)
            })
        }
