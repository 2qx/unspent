...
describe(`Record Contract Tests`, () => {
    ...
    test("Should record a division contract.", async () => {
        ...
        let regTest = new ElectrumCluster('utxfi-tests - record', '1.4.1', 1, 1, ClusterOrder.PRIORITY);
        ...
        await contract!.functions['execute']!(checkHash)
            ...
            .send();
        expect(await contract.getBalance()).toBeGreaterThanOrEqual(50000 - (test.length/2));
    });
