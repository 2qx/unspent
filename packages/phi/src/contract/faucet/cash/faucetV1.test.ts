...
describe(`Faucet Contract Tests`, () => {
    ...
    test("Should pay a faucet contract 5 times in 5 blocks", async () => {
        ...
        let regTest = new ElectrumCluster('utxfi-tests - faucet', '1.4.1', 1, 1, ClusterOrder.RANDOM, 5000);
        ...
        for (let x = 0; x < 5; x++) {
            ...
            let size = (await contract!.functions
                .drip()
                .to([
                    {
                        to: contract.address,
        ...
        expect(await bob.getBalance("sat")).toBeGreaterThanOrEqual((payout - 200)*5);
    });
