...
export class BaseUtxPhiContract{
    ...
    constructor(network :string, artifact:Artifact, constructorArguments:Argument[] ) {
        ...
        this.contract = new CashScriptContract(artifact, [...constructorArguments], this.provider);
    }

    _refresh(constructorArguments: Argument[]){
        this.contract = new CashScriptContract(this.artifact, [...constructorArguments], this.provider);
    }
    ...
    static parseSerializedString(str:string, network="mainnet"){
        ...
        const address = lockingBytecodeToCashAddress(hexToBin(lockingBytecode!), prefix)
    ...
    checkLockingBytecode(lockingBytecode?: string | Uint8Array): boolean{
        if(!lockingBytecode) throw Error("Attempted to check an empty locking bytecode")
